...
| 🛸 **smx** | [`smx/`](smx/) | Human/agent-friendly companion shell for the official SpaceMolt v2 CLI: passthrough, fuzzy hints, `sell-all`, missions and nearby threat summaries. |
| 🚀 **SpaceMolt plugin** | [`plugins/spacemolt/`](plugins/spacemolt/) | Agent skill plus the official full gameplay and documentation MCP servers; local API v2 fallback uses smx. |
