...
The GitHub **Latest** release exposes one Feedboard download: [`feedboard.zip`](https://github.com/travnie/twojstar/releases/latest/download/feedboard.zip). Extract it and double-click `Install-Feedboard.cmd`. The GitHub sideload path requires an authenticated GitHub CLI (`gh auth login`) so the installer can verify the published artifact attestations before it trusts `Feedboard.cer` in `LocalMachine\TrustedPeople`; it then installs the bundled dependencies and Feedboard.
